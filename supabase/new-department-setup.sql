-- ============================================================
-- ASES — New Department Database Setup
-- ============================================================
-- Run this ONCE, in full, in the SQL Editor of a brand-new, EMPTY
-- Supabase project — one project per department. It recreates the
-- exact same schema (tables, types, functions, triggers, RLS
-- policies) that the DMS project uses, plus that department's own
-- branding row, so the same Next.js frontend works against it
-- unchanged.
--
-- Pre-filled below for: DMMC — Department of Multimedia and Mass
-- Communication. To set up a different/future department instead,
-- just edit the four `department_name` / logo / edge-function values
-- in STEP 7 near the bottom before running.
--
-- After running this file:
--   1. Project Settings → API → copy the Project URL and anon/publishable
--      key into NEXT_PUBLIC_SUPABASE_URL_DMMC / NEXT_PUBLIC_SUPABASE_ANON_KEY_DMMC
--      in .env.local (see .env.local.example).
--   2. Create the first admin: Authentication → Add user (email + password),
--      then run the STEP 8 statement below with that user's UUID to make
--      them a super_admin.
--   3. Deploy an `admin-create-user` Edge Function to this project (same
--      one the DMS project uses) if you want the in-app "Add User" screen
--      to work here too, then paste its URL into app_settings.edge_function_base_url.
-- ============================================================
...
-- ============================================================
-- 🏷️ STEP 7: App Settings (branding shown in sidebar & PDF exports)
-- ============================================================
-- EDIT the department_name (and logos / edge_function_base_url if
-- needed) below before running, if this isn't for DMMC.
...
CREATE TABLE public.app_settings (
  ...
  department_name text NOT NULL DEFAULT 'Department of Multimedia and Mass Communication',
...
INSERT INTO public.app_settings (
  ...
  -- ⚠️ Paste this department's own admin-create-user Edge Function URL here
  -- once deployed to THIS Supabase project (see notes at top of this file).
  -- Until then, the "Add User" screen in /users will not work for this department.
  ''
);
...
-- ============================================================
-- 🔑 STEP 8: Make your first admin a super_admin
-- ============================================================
-- 1. In this Supabase project: Authentication → Users → Add user
--    (enter their email + a temporary password, or invite by email).
-- 2. Copy that user's UUID from the Users table.
-- 3. Uncomment the line below, paste the UUID and their name in, and run it:
--
-- INSERT INTO public.admin_users (id, full_name, role, is_active)
-- VALUES ('PASTE-USER-UUID-HERE', 'Full Name', 'super_admin', true);
